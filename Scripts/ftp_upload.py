...
def monitor_and_ftp(server, username, password, local_dir, remote_dir, walk=False):
...
def upload_all(server, username, password, base_local_dir, base_remote_dir, files_to_update=None, walk=False):
...
if __name__ == '__main__':
    ...
    if (options.username and options.server and options.remote_dir) or \
        ...
        if monitor:
            try:
                monitor_and_ftp(server,username,p,local_dir, remote_dir, walk)
            except KeyboardInterrupt:
                print 'Exiting...'
        else:
            try:
                upload_all(server, username, p, local_dir, remote_dir, [], walk)
            except KeyboardInterrupt:
                print 'Exiting...'
